...
import { createSqlitePdo } from '../../src';
import SqliteDriver from '../../src/sqlite-driver';
...
describe('Sqlite Driver', () => {
  ...
  it('Works Wal', async () => {
    let pdo = new Pdo(pdoData.driver, {
      ...pdoData.config,
      path: ':memory:',
      wal: true,
    });
  ...
  it('Works Synchronous Option Only When Wal Enabled', async () => {
    ...
    await pdo.disconnect();
    pdo = new Pdo(pdoData.driver, {
      ...pdoData.config,
      walSynchronous: 'NORMAL',
    });
  ...
  it('Works Wal Max Size', async () => {
    ...
    expect(
      messages[1].message.startsWith(
        'WAL checkpoint TRUNCATE called, file size',
      ),
    ).toBeTruthy();
  ...
  it('Works Wal Always False When Memory Database', async () => {
    ...
    const pdo = new Pdo(pdoData.driver, {
      ...pdoData.config,
      path: ':memory:',
      wal: true,
      walMaxSize: 0.01,
  ...
  it('Works Exec Return Number', async () => {
    ...
    expect(
      await trx.exec(
        "INSERT INTO users (name, gender) VALUES ('Claudio', 'All');",
      ),
    ).toEqual(1);
    await trx.rollback();
  });
