...
describe('Sqlite BigInt Cast', () => {
  it('Works Cast', async () => {
    ...
    let stmt = await pdo.query(
      "SELECT CAST('9007199254740992' as SIGNED INTEGER)",
    );
    ...
    stmt = await pdo.query(
      "SELECT CAST('-9007199254740991' as SIGNED INTEGER)",
    );
    expect(stmt.fetchColumn(0).get()).toEqual(-9007199254740991);

    // precision is lost bigint is returned without precision
    stmt = await pdo.query(
      'select 111111111111111111111111111111111111111111111111 as abc',
    );
