...
describe('Sql Prepared Statement', () => {
  ...
  it('Works Statement Bind Value Fails With Mixed Values', async () => {
    let stmt = await pdo.prepare(
      'SELECT * FROM users where gender = ? LIMIT :limit;',
    );
  ...
  it('Works Statement Bind Typed Binding', async () => {
    ...
    stmt.bindValue('limit', new TypedBinding(PARAM_BIGINT, BigInt(3)));
    await stmt.execute();
    expect(stmt.fetchArray().all().length).toBe(3);
    await stmt.close();
  ...
  it('Works Statement Bind Date', async () => {
    ...
    expect(new Date((stmt.fetchColumn(0).get() as number) * 1000)).toEqual(
      date,
    );
    await stmt.close();
  });
  ...
  it('Works Statement Bind Buffer', async () => {
    ...
    expect(stmt.fetchColumn<Buffer>(0).get()?.toString()).toBe(
      'buffer as blob on database',
    );
    await stmt.close();
    ...
    await stmt.close();
    expect(
      await pdo.exec('DELETE FROM companies WHERE (id = ' + lastId + ');'),
    ).toBe(1);
  });
