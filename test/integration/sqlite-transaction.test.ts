import {
  ...
  PdoStatement,
  PdoTransaction,
  PdoTransactionPreparedStatement,
} from 'lupdo';
...
describe('Sqlite Transactions', () => {
  ...
  it('Works Transaction Rollback && Commit', async () => {
    ...
    const stmt = await pdo.query(
      "SELECT id FROM users where name = 'Claudio';",
    );
    const id = stmt.fetchColumn<number>(0).get() as number;
    expect(await pdo.exec('DELETE FROM users WHERE (id = ' + id + ');')).toBe(
      1,
    );
  });
